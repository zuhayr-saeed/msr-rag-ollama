/**
/** UIMA + embed: same as RunDeltaOnce plus embedding delta pass. */
...
import edu.yourorg.msr.uima.{NormalizeAnnotator, DeltaGateAnnotator, ChunkAnnotator, PersistAnnotator, EmbedAnnotator}

object RunDeltaEmbedOnce extends edu.yourorg.msr.util.Logging {
  def main(args: Array[String]): Unit = {
    val input = sys.env.getOrElse("RAG_INPUT_DIR",
      throw new RuntimeException("Set RAG_INPUT_DIR to your corpus path"))
    val root = Paths.get(input); require(Files.exists(root), s"Path not found: $input")

    val pdfs = Files.walk(root).iterator().asScala
      .filter(Files.isRegularFile(_)).filter(_.toString.toLowerCase.endsWith(".pdf"))
      .take(10).toVector

    log.info(s"Running delta+embed pipeline on ${pdfs.size} PDF(s) ...")
    ...
    val embed     = AnalysisEngineFactory.createEngine(AnalysisEngineFactory.createEngineDescription(classOf[EmbedAnnotator]))

    var processed, skipped, totalChunks = 0
    ...
    pdfs.foreach { p =>
      ...
      if (doc.getBooleanValue(fSkip)) {
        ...
        totalChunks += jCas.getAnnotationIndex(chunkType).size()
        processed += 1
      }
    }
    ...
    log.info(s"  chunks (CAS)   : $totalChunks")
    log.info("\nDone.")
  }
}
